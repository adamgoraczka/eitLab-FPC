...
function newCab(){ return {id:uid(), w:750, h:2000, plinth:0, door:false, items:[]}; }
...
function renderCanvas(){
  ...
  P.cabinets.forEach((c,ci)=>{ const x0=cabX(c); const on=c===selCab; const e=Math.max(4,c.w*0.012);
    ...
    if(MODE==='door'||c.door){ s+=`<rect x="${x0+e*0.6}" y="${e*0.6}" width="${c.w-e*1.2}" height="${c.h-e*1.2}" rx="${e*0.4}" fill="var(--door)" stroke="var(--cab-edge)" stroke-width="1" vector-effect="non-scaling-stroke"/><rect x="${x0+c.w-e*3.2}" y="${c.h/2-e*2.5}" width="${e*0.9}" height="${e*5}" rx=".5" fill="var(--cab-edge)"/></g>`; return; }
...
function renderCabs(){ $('cabs').innerHTML=P.cabinets.map((c,i)=>`<div class="it${c===selCab?' on':''}" data-cab="${c.id}"><span class="grow"><b>Szafa ${i+1}</b> · ${c.w}×${c.h}${c.plinth?' +'+c.plinth:''} · ${c.items.length} el.</span><button class="small ghost icon" data-act="left" title="W lewo">◀</button><button class="small ghost icon" data-act="right" title="W prawo">▶</button><button class="small ghost icon" data-act="dup" title="Duplikuj">⧉</button><button class="small danger icon" data-act="del">×</button></div>`).join('')||'<div class="it">Brak szaf</div>';
  const c=selCab; $('cabEdit').innerHTML= c? `<div class="row three" style="margin-top:8px"><label class="f">Szerokość mm<input type="number" data-cf="w" value="${c.w}" step="10"></label><label class="f">Wysokość mm<input type="number" data-cf="h" value="${c.h}" step="10"></label><label class="f">Cokół / fundament mm<input type="number" data-cf="plinth" value="${c.plinth||0}" step="10" min="0"></label></div><label class="chk"><input type="checkbox" data-cf="door" ${c.door?'checked':''}> Pokazuj z zamkniętymi drzwiami</label>`:''; }
...
function migrateBoard(b){ if(b.rows&&!b.cabinets){ const c=newCab(); c.w=(b.front&&b.front.w)||600; c.h=(b.front&&b.front.h)||800; const L=Object.assign(defaultLabel(),b.label||{}); let y=0;
  ...
  for(const c of B.cabinets){ if(c.plinth==null) c.plinth=0; for(const it of c.items) if(it.type==='cover'){ const r=it.row; if(!r.groups){ const groups=[]; let cur=null; for(const d of (r.devices||[])){ const gn=d.group||''; if(!cur||cur.name!==gn){ cur=newGroup(gn,'left'); groups.push(cur); } cur.devices.push(d); } r.groups=groups; delete r.devices; }
...
$('cabEdit').addEventListener('change',e=>{ const k=e.target.dataset.cf; if(!k||!selCab) return; snapshot(); if(k==='door') selCab.door=e.target.checked; else if(k==='plinth') selCab.plinth=Math.max(0,+e.target.value||0); else selCab[k]=Math.max(10,+e.target.value||10); change(); });
// items
...
SZAFA; szerokość; wysokość; cokół                 – nowa szafa; cokół w mm (0 = brak)
...
function parseCsv(text){
  ...
  text.replace(/^\uFEFF/,'').split(/\r?\n/).forEach((raw,ln)=>{ const l=raw.trim(); if(!l||l.startsWith('#')) return; const cells=(l.includes(';')?l.split(';'):l.split('\t')).map(c=>c.trim()); const k=norm(cells[0]); const a=cells.slice(1);
    ...
    switch(k){
      ...
      case 'SZAFA': case 'CABINET': cab=newCab(); cab.w=num(a[0],750); cab.h=num(a[1],2000); cab.plinth=num(a[2],0); out.cabinets.push(cab); cover=null; grp=null; break;
