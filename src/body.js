...
function doorSvg(c,x0,e){ const leaves=c.leaves===2?2:1; const hinge=c.hinge==='right'?'right':'left'; const m=e*0.6; const H=c.h-2*m; let s='';
  const handle=(hx,hy,dir)=>{ const u=Math.max(8,Math.min(22,c.w/32)); const bx=hx-u*0.5; return `<rect x="${bx}" y="${hy-u*2.2}" width="${u}" height="${u*4.4}" rx="${u*0.3}" fill="var(--cab-edge)" opacity=".85"/><circle cx="${hx}" cy="${hy-u*1.4}" r="${u*0.3}" fill="var(--door)"/><rect x="${dir>0?hx-u*0.2:hx-u*2.6}" y="${hy-u*0.35}" width="${u*2.8}" height="${u*0.7}" rx="${u*0.3}" fill="var(--cab-edge)"/><circle cx="${hx}" cy="${hy+u*1.5}" r="${u*0.32}" fill="none" stroke="var(--door)" stroke-width="${u*0.12}"/>`; };
  ...
  if(leaves===1){ s+=`<rect x="${x0+m}" y="${m}" width="${c.w-2*m}" height="${H}" rx="${e*0.4}" fill="var(--door)" stroke="var(--cab-edge)" stroke-width="1.2" vector-effect="non-scaling-stroke"/>`; s+=`<rect x="${x0+m+e*1.2}" y="${m+e*1.2}" width="${c.w-2*m-e*2.4}" height="${H-e*2.4}" rx="${e*0.3}" fill="none" stroke="var(--cab-edge)" stroke-width=".4" opacity=".5" vector-effect="non-scaling-stroke"/>`;
    const hx= hinge==='left'? x0+c.w-m-e*2.6 : x0+m+e*2.6; s+=handle(hx, c.h*0.5, hinge==='left'?-1:1); s+=hinges(hinge==='left'? x0+m : x0+c.w-m); }
    ...
    s+=handle(x0+m+lw-e*2.2, c.h*0.5, -1); s+=`<rect x="${x0+m+lw+e*1.2}" y="${c.h*0.5-e*2.2}" width="${e*0.9}" height="${e*4.4}" rx=".5" fill="var(--cab-edge)" opacity=".85"/>`; s+=hinges(x0+m)+hinges(x0+c.w-m); }
  return s; }
